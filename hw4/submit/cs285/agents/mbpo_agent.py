...
class MBPOAgent(BaseAgent):
    ...
    def collect_model_trajectory(self, rollout_length=1):
        ...
        ob, _, _, _, terminal = self.mb_agent.replay_buffer.sample_random_data(1) #self.sample(1) #TODO
        ...
        for _ in range(rollout_length):
            ...
            next_ob = np.mean([model.get_prediction(ob, ac, self.mb_agent.data_statistics) for model in self.mb_agent.dyn_models], axis=0)
            #next_ob = calculate_mean_prediction_error(ob, ac, self.mb_agent.dyn_models, self.mb_agent.data_statistics)
            #mean_pred = np.mean(ac,axis=0) #np.random.choice(num_data, size=num_data_per_env, replace=False) #TODO
