...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, initial_expertdata, collect_policy, num_transitions_to_sample, save_expert_data_to_disk=False):
        ...
        if itr == 0 and initial_expertdata is not None:
            with (open(initial_expertdata, "rb")) as openfile:
                loaded_paths = pickle.load(openfile)
            ...
            while envsteps_this_batch <= batch_size:
                paths.extend(utils.sample_n_trajectories(self.env,
                    collect_policy,
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            #print('Amount of samples:',self.params['train_batch_size'])
            # TODO sample some data from the data buffer
            # HINT1: use the agent's sample function
            # HINT2: how much data = self.params['train_batch_size']
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = self.agent.sample(self.params['train_batch_size'])

            #print('Observations',ob_batch)
