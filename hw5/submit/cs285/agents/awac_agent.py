...
class AWACAgent(DQNAgent):
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            r_explore = self.exploration_model.forward_np(next_ob_no)
            #expl_std = self.running_rnd_rew_std

            #print('NEW STD:',self.running_rnd_rew_std)
            #print('bonus',expl_bonus)

            #self.running_rnd_rew_std = (self.running_rnd_rew_std*self.rnd_gamma) + (np.std(r_explore)*(1-self.rnd_gamma))
            r_std = np.std(r_explore)
            r_explore = normalize(r_explore, 0, self.running_rnd_rew_std) 

            self.running_rnd_rew_std = (self.running_rnd_rew_std*self.rnd_gamma) + (r_std*(1-self.rnd_gamma))
            ...
            mixed_reward = explore_weight * r_explore + exploit_weight * re_n 
